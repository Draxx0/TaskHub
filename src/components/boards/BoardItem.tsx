  ...
  <Card className="hover:border-main-500 transition ease-in-out duration-300">
   <CardHeader className="p-0">
    <div className="p-4 flex flex-col gap-3">
     ...
     <CardDescription className="truncate" title={board.description}>{board.description}</CardDescription>
    </div>
   </CardHeader>
   <CardContent className="p-4">
    <p>{t("workshop_card.board")} - <span className="text-main-500 font-semibold">4</span></p>
   </CardContent>
   <CardFooter className="flex p-4 justify-between">
    <small> {t("workshop_card.date")} <span className="font-semibold">{formatDate({ date: board.createdAt, locale: storedLang || "fr" })}</span></small>
    <Button variant={"outline"} onClick={() => navigate(`/workshops/${workshopId}/boardId/${board.id}`)}>
