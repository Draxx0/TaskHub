...
const BoardsList = () => {
 const [currentWorkshopRef, setCurrentWorkshopRef] = useState<DocumentReference<DocumentData, DocumentData> | undefined>(undefined);
 const { id } = useParams();
 ...
 const { data: boards, isLoading, isError } = useGetCollection<Board>({
  path: "boards",
  condition: {
 ...
 return (
  <div className={`grid grid-cols-4 gap-8 ${boards && boards.length === 0 && "relative h-[65vh]"}`}>
   {isLoading ? (
    <>
     <SkeletonCard />
     <SkeletonCard />
    ...
    boards.length > 0 ? (
     ...
     <p className="text-black/75">Vous n'avez pour l'instant aucun tableaux</p>
    </div>
   ) : isError ? (
    <Error />
   ) : null}
